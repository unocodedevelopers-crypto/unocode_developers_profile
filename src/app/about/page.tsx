    ...
    <>
      <div className="relative flex min-h-screen w-full flex-col gap-20 overflow-x-clip"><Header />
        <div className="flex flex-col items-center pt-32 px-5 lg:px-10 gap-16 md:gap-32 w-full max-w-[1200px] mx-auto">
          ...
          {/* Dynamic Sections Before Process */}
          {sections.filter(s => s.id !== "process" && s.id !== "vision").map((section) => 
            renderSection(section, sections.findIndex(s => s.id === section.id))
          )}
        </div>

        <ProcessSection />

        <div className="flex flex-col items-center px-5 lg:px-10 gap-16 md:gap-32 w-full max-w-[1200px] mx-auto">
          {/* Dynamic Sections After Process */}
          {sections.filter(s => s.id === "vision").map((section) => 
            renderSection(section, sections.findIndex(s => s.id === section.id))
          )}
